...
continue_or_exit() {
  if [ "$GUIDE_MODE" = true ]; then
    ...
    read -p "${COLOR_CYAN}Hint: Press Enter to continue, or 'q' to quit: ${COLOR_RESET}" choice
    if [[ "$choice" == "q" ]]; then
...
check_existing_repo() {
  ...
  if git rev-parse --is-inside-work-tree &> /dev/null; then
    ...
    read -p "${COLOR_CYAN}Hint: Enter your choice (1, 2, or 3): ${COLOR_RESET}" repo_choice
    ...
    case $repo_choice in
      ...
      2)
        ...
        read -p "${COLOR_RED}Type 'YES' (in uppercase) to confirm deletion: ${COLOR_RESET}" confirm_delete
...
initialize_git() {
  ...
  if [ "$GUIDE_MODE" = true ]; then
    ...
    read -p "${COLOR_CYAN}Hint: Do you want to initialize a new Git repository here? (Y/n): ${COLOR_RESET}" confirm_init
...
add_files_to_staging() {
  if [ "$GUIDE_MODE" = true ]; then
    ...
    read -p "${COLOR_CYAN}Hint: Do you want to add all files to the staging area? (Y/n): ${COLOR_RESET}" confirm_add
...
make_initial_commit() {
  ...
  if [ "$GUIDE_MODE" = true ]; then
    ...
    read -p "${COLOR_CYAN}Hint: Do you want to make the initial commit? (Y/n): ${COLOR_RESET}" confirm_commit
...
prompt_for_remote_url() {
  ...
  read -p "${COLOR_CYAN}Hint: Enter the GitHub repository URL: ${COLOR_RESET}" REMOTE_URL
...
add_or_update_remote() {
  ...
  if git remote get-url origin &> /dev/null; then
    ...
    if [ "$EXISTING_REMOTE_URL" == "$REMOTE_URL" ]; then
      ...
      if [ "$GUIDE_MODE" = true ]; then
        read -p "${COLOR_CYAN}Hint: Remote 'origin' already exists and points to a different URL ($EXISTING_REMOTE_URL). Do you want to update it to $REMOTE_URL? (y/N): ${COLOR_RESET}" UPDATE_REMOTE
...
push_to_remote() {
  if [ "$GUIDE_MODE" = true ]; then
    ...
    read -p "${COLOR_CYAN}Hint: Do you want to push your project to the remote repository now? (Y/n): ${COLOR_RESET}" confirm_push
